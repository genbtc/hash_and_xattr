#!/bin/bash
# openssl-hash-sign.sh v0.2 by genBTC 2025
...
sig_step6_raw="$INPUT_FILE.message"         #3af28d
...
echo "--------"
# Step 2: RSA sign the digest with your private key
echo "Signing the digest with RSA private key: $PRIVATE_KEY"
...
# Step 3b: Verify and recover the signature with the public key
...
if [ -s "$step4" ]; then
    ...
    # Step 5: Compare the recovered hash with the original hash
...
openssl pkeyutl -sign -digest sha512 -inkey "$PRIVATE_KEY" -rawin -in "${INPUT_FILE}" -out "${sig_step6_raw}"
# Step 6a: Print the signature (in hex format)
echo "Signature (raw):"
xxd -p -c 64 "${sig_step6_raw}"     # 12d507
# Step 6b: Verify and recover the signature with the public key
echo "VerifyRecovering the original hash using the RSA public key: $PUBLIC_KEY"
openssl pkeyutl -verify -digest sha512 -pubin -inkey "$PUBLIC_KEY" -rawin -in "${INPUT_FILE}" -sigfile "${sig_step6_raw}"
